...
下面这些词尽量按“看页面时怎么理解”来解释：

- Basic AWGN：可以理解成“只有背景噪声的简单无线环境”，适合先看基础效果。
- Advanced CDL+OFDM：可以理解成“更接近真实场景的复杂无线环境”，通常更贴近实测，但计算也更重。
- Eb/No：可以理解成“信号比噪声强多少”，越高说明信号越清楚。
- BER：无线传输出错的比例，越低越好。
- 无线时延：一次命令从发出到能用的时间，越低越快。
- 轨迹误差：目标动作和实际动作的偏差，越低越准。
- 端到端总时延：命令从发送端传到接收端的总耗时，越低越顺手。
- 丢包率：传输过程中丢掉的数据比例，越低越稳定。
- 抖动：时延的波动大小，越小越不容易忽快忽慢。
- 繁忙度：路由器当前有多“堵”，数值越高，越容易排队。
